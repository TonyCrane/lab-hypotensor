...
另一种情况来说，所见即所得的 markdown 编辑器一般都会遵从按一次 <kbd>Enter</kbd> 换行就是新起一段的使用逻辑（而带有附加按键比如 <kbd>Shift+Enter</kbd> 则是换行不新起一段），所以在转为直接编辑 markdown 源码的时候就会有人忽略空行的作用以及“新起一段”和“换行”的区别。
